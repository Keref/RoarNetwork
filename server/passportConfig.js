...
passport.use(new BearerStrategy(
	function(token, done) {
		console.log('login bearer')
		User.findOne({ api_token: token }, function (err, user) {
			console.log('try to log with token', token)
...
exports.isAuthorized = (req, res, next) => {
	...
	if (token) {
		...
		if (
			token.accessTokenExpires && moment(token.accessTokenExpires).isBefore(moment().subtract(1, 'minutes'))
		) {
		...
		(req, accessToken, tokenSecret, profile, done) => {
			if (req.user) {
				...
				User.findOne({ twitter: profile.id }, (err, existingUser) => {
					...
					user.username = `${profile.username}@twitter.com`;
					user.twitter = profile.id;
					user.tokens.push({ kind: 'twitter', accessToken, tokenSecret });

					// console.log('twitterloggedin', profile)
					// TODO: check if username taken onchain, otherwise can take it
