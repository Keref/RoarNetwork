...
class LoginModal extends React.Component {
  ...
  getCode = async () => {
	  ...
	  var res = await roarAPI.getCode({phone: this.state.phone, countryCode: this.state.country.phone});
	  //start timer
  }
  sendCode = async () => {
	  ...
	  var profile = await roarAPI.loginWithCode({phone: this.state.phone, countryCode: this.state.country.phone, phone2fa: this.state.phone2fa});
	  if (profile.status == 'success') {
		  // setup wallet
  ...
  render() {
	...
	let fullScreen = !this.props.mediaQuery;
    return (
      <Dialog
