...
class ProfilePage extends React.Component {
  ...
  buySellTokens = async () => {
    ...
    if (this.state.buyOrSell == 'buy') {
      const amountBought = await this.context.wallet.buyTokens(
        this.state.amount,
        this.state.profile.profileAddress,
      );
    } else {
      const valueSold = await this.context.wallet.sellTokens(
        this.state.amount,
        this.state.profile.profileAddress,
      );
    }
  ...
  render() {
    const sharePrice = parseFloat(
      this.state.profile.tokenValue
        ? this.state.profile.tokenSupply / this.state.profile.tokenValue
        : 0,
    ).toFixed(2);
    const peratio =
      this.state.profile.lifetimeDividends > 0 &&
      this.state.profile.tokenValue / this.state.profile.lifetimeDividends;
    return (
      <div
        style={{
        ...
        <div>
          ...
          >
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              >
                ...
                <span style={{ fontSize: 18, fontWeight: 'bold' }}>
                  {parseFloat(
                    (this.state.profile.tokenSupply || 0) / 10 ** 18,
                  ).toFixed(2)}{' '}
                </span>
              </span>
              <span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              <span>Avail: 0</span>
              <Button onClick={this.buySellTokens}>
